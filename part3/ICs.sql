...
AFTER INSERT OR UPDATE ON sailor INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE check_sailor_integrity();

CREATE CONSTRAINT TRIGGER tg_check_junior
AFTER INSERT OR UPDATE OR DELETE ON junior INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE check_sailor_integrity();

CREATE CONSTRAINT TRIGGER tg_check_senior
AFTER INSERT OR UPDATE OR DELETE ON senior INITIALLY DEFERRED
FOR EACH ROW EXECUTE PROCEDURE check_sailor_integrity();
--------- END OF (IC-1) -----------
