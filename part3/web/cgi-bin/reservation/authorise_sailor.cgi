...
def authorise_sailor():
    try:
        ...
        sailor_query = "SELECT email FROM sailor EXCEPT SELECT sailor FROM authorised as a WHERE a.start_date = %s AND a.end_date = %s AND a.boat_country = %s AND a.cni = %s;"
        cursor.execute(sailor_query, (start_date, end_date, country, cni))
        sailors = cursor.fetchall()

        form = get_form( action=f"authorise_sailor_update.cgi?start_date={start_date}&\
end_date={end_date}&country={country}&cni={cni}",
                        selects=[dict(name='sailor', options=sailors, label="Sailor email")])
